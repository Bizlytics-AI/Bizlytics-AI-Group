...
async def upload_file(
    ...
    hr_account = db.query(HRAccount).filter(HRAccount.email == current_user.email).first()
    if not hr_account:
...
def list_company_files(
    ...
    files = db.query(RawUpload).filter(
        RawUpload.company_id == hr_account.company_id
    ).order_by(RawUpload.created_at.desc()).all()
