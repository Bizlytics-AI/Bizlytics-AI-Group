...
def list_pending_companies(db: Session = Depends(get_db), current_admin: User = Depends(require_admin)):
...
def approve_pending_company(company_id: int, db: Session = Depends(get_db), current_admin: User = Depends(require_admin)):
...
def reject_pending_company(company_id: int, db: Session = Depends(get_db), current_admin: User = Depends(require_admin)):
